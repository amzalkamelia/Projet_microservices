...
import com.codewithus.siteservice.Exception.RessourceNotFoundException;
...
public class SiteServiceImpl implements SiteService{
    ...
    public SiteDto getSiteById(Long siteId) {
        Site site = siteRepository.findById(siteId)
                .orElseThrow(() -> new RessourceNotFoundException("Site does not exist with given id : " + siteId));
    ...
    public SiteDto updateSite(Long id, SiteDto siteDto) {
        Site site = siteRepository.findById(id).orElseThrow(
                () -> new RessourceNotFoundException("Site does not exist with the given id : " + id)
        );
    ...
    public SiteDto getSitesByName(String name) {
        Site site = siteRepository.findByName(name)
                .orElseThrow(() -> new RessourceNotFoundException("Site not found with name: " + name));
        return SiteMapper.mapToSiteDto(site);
    }
    ...
    public SiteDto updateSiteField(Long id, String fieldName, Object fieldValue) {
        Site site = siteRepository.findById(id)
                .orElseThrow(() -> new RessourceNotFoundException("Site does not exist with the given id: " + id));
